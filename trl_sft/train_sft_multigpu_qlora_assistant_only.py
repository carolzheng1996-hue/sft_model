#!/usr/bin/env python3
"""Multi-GPU QLoRA SFT for a local Qwen2.5 model with assistant-only loss.

Launch with accelerate or torchrun, for example:

  accelerate launch --num_processes 4 train_sft_multigpu_qlora.py ...

The number of GPUs is controlled by the launcher, not by SFTTrainer args.
Each distributed process loads the 4-bit model onto its own LOCAL_RANK device,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no_4bit", action="store_false", dest="use_4bit", help="Disable 4-bit loading.")
    ...
    parser.set_defaults(use_4bit=True, assistant_only_loss=True)
...
def qlora_device_map() -> dict[str, int] | str:
    """Map the whole quantized model to this process's GPU under DDP."""
...
def main() -> None:
    ...
    if is_main_process():
        ...
        print(
            f"Starting {mode} QLoRA SFT: world_size={get_world_size()}, "
