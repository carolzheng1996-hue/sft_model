#!/usr/bin/env python3
"""Parallel full-set evaluation for local Time-MQA messages JSON on Ascend NPU."""
...
from npu_utils import get_inference_dtype, is_npu_available, preferred_distributed_backend, setup_npu


DEFAULT_BASE_MODEL_PATH = "../models/Qwen2.5-1.5B"
DEFAULT_ADAPTER_PATH = "outputs/qwen2.5-1.5b-timemqa-local-multigpu-lora-npu"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--load_4bit", action="store_true", help="Unsupported on Ascend NPU; raises if set.")
...
def eval_device():
    ...
    rank = local_rank()
    if is_npu_available():
        return torch.device("npu", rank if rank >= 0 else 0)
    if torch.cuda.is_available():
        if rank >= 0:
            torch.cuda.set_device(rank)
        return torch.device("cuda", rank if rank >= 0 else 0)
...
def load_model_and_tokenizer(args: argparse.Namespace):
    ...
    if args.load_4bit:
        raise ValueError("Ascend NPU evaluation does not support bitsandbytes 4-bit loading.")
