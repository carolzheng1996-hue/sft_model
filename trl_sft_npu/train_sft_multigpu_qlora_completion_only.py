#!/usr/bin/env python3
"""Multi-NPU LoRA SFT with completion-only loss via a data collator.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TRL LoRA SFT with completion-only loss.")
    ...
    parser.set_defaults(use_4bit=False, drop_overlong_assistant=True)
...
def main() -> None:
    try:
        ...
        maybe_login()
        if is_main_process():
            print(f"Starting completion-only LoRA SFT: world_size={get_world_size()}, output_dir={args.output_dir}")
